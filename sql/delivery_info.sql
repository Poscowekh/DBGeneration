...
from (
    ...
    join (
        select id, customer_id, delivery_comment
        from dryclean.orders
        where actual_finish_date is null and
              to_be_delievered
        ) as o on cl.order_id = o.id
...
group by co.id, co.name, co.phone_number, t.id,
         o.id, delivery_comment, cl.order_id,
         cu.id, cu.name, cu.address, cu.phone_number

order by co.id, cu.id, o.id;
